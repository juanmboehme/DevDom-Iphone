...
function iScroll (el, options) {
	...
	that.options = {
		...
		shouldPreventDefault: function (el) { return true; }
	};
	...
	that.onUserScroll = that.options.onUserScroll;
	delete that.options.onScrollEnd;
	delete that.options.onScrollStart;
	delete that.options.onUserScroll;
...
iScroll.prototype = {
	...
	handleEvent: function (e) {
		...
		switch (e.type) {
			...
			case 'DOMSubtreeModified':
				...
				break;
			case 'click':
				if (that.lastMouseUpTimeStamp &&
					(e.timeStamp - that.lastMouseUpTimeStamp) < 50) {
					e.preventDefault();
					e.stopPropagation();
				}
				break;
	...
	touchStart: function(e) {
		...
		if (that.options.shouldPreventDefault(e.target)) {
			e.preventDefault();
			e.stopPropagation();
		} else {
			return;
...
//			that.dist+= Math.abs(leftDelta) + Math.abs(topDelta);
	...
	touchEnd: function(e) {
		...
		that.lastMouseUpTimeStamp = e.timeStamp;
		that.onUserScroll();
...
function scrollbar (dir, wrapper, fade, shrink, color) {
	...
	that.bar.style.cssText = 'position:absolute;top:0;left:0;-webkit-transition-timing-function:cubic-bezier(0,0,0.25,1);pointer-events:none;-webkit-transition-duration:0;-webkit-transition-delay:0;-webkit-transition-property:-webkit-transform;z-index:10;border:1px solid rgba(127, 127, 127, 0.5);background:' + color + ';' +
